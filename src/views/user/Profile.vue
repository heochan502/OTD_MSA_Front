...
import { getUserProfile, patchUserProfilePic, deleteUserProfilePic, changePassword } from '@/services/userService'
...
const loadProfile = async () => {
  ...
  try {
    ...
    if (response.data.resultCode === 1) {
      ...
      throw new Error(response.data.message || '프로필 정보를 불러올 수 없습니다.')
    }
  } catch (err) {
    console.error('프로필 로딩 에러:', err)
    error.value = err.message || '프로필 정보를 불러오는 중 오류가 발생했습니다.'
  } finally {
