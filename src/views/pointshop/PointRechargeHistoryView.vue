...
const fetchRechargeHistory = async () => {
  try {
    ...
    rechargeHistory.value = Array.isArray(res.data) ? res.data : res.data?.data || [];
  } catch (err) {
...
<template>
  <div class="recharge-history">
    ...
    <table v-else>
      <thead>
        <tr><th>충전일</th><th>충전 금액</th><th>잔액</th></tr>
      </thead>
      ...
      <tbody v-else>
        <tr><td colspan="3" class="no-data">충전 이력이 없습니다.</td></tr>
      </tbody>
    </table>
  </div>
</template>
...
<style scoped>
.recharge-history { max-width: 800px; margin: 40px auto 0; padding: 20px; background-color: #fafafa; border-radius: 12px; box-shadow: 0 2px 6px rgba(0, 0, 0, 0.05); }
.title { font-size: 1.3rem; font-weight: bold; color: #222; text-align: center; margin-bottom: 20px; }
table { width: 100%; border-collapse: collapse; background-color: white; border-radius: 8px; overflow: hidden; }
th, td { padding: 12px 10px; border-bottom: 1px solid #ddd; text-align: center; font-size: 0.95rem; }
th { background-color: #f2f2f2; color: #444; font-weight: 600; }
tr:hover td { background-color: #f9f9f9; }
.loading-message, .no-data { text-align: center; padding: 20px; color: #666; font-size: 0.95rem; }
</style>
